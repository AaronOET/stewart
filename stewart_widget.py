...
class StewartPlatformWidget:
    ...
    def _build_figure(self):
        ...
        self.ax_time.set_xlabel("Time (sec)")
        self.ax_time.set_ylabel("Disp (mm)")
    ...
    def _init_graphics(self):
        ...
        (self.line_base,) = self.ax_3d.plot(
            base_radius * np.cos(th),
            base_radius * np.sin(th),
            np.full_like(th, bz[0]),
            "k--",
        )
        ...
        self.lines_legs = [
            self.ax_3d.plot([], [], [], color="gray", linewidth=3)[0] for _ in range(6)
        ]
        self.scat_plat = self.ax_3d.scatter([], [], [], c="b", s=60, depthshade=True)
        (self.line_plat,) = self.ax_3d.plot([], [], [], "b-", linewidth=1.5)
        self.scat_center = self.ax_3d.scatter(
            [0], [0], [0], c="b", s=30, edgecolors="none", zorder=100
        )
        (self.line_path,) = self.ax_3d.plot(
            [], [], [], color="m", alpha=0.3, linewidth=1
        )
        self.scat_fail = self.ax_3d.scatter(
            [], [], [], c="r", marker=".", s=1, zorder=50
        )
    ...
    def _build_controls(self):
        ...
        self.chk_safe = widgets.Checkbox(
            value=False, description="Limit Stroke to 80%", indent=False
        )
        self.chk_safe.observe(self._on_safety_change, names="value")

        self.chk_zoom = widgets.Checkbox(
            value=False, description="Zoom to Center (±300 mm)", indent=False
        )
        self.chk_zoom.observe(self._on_zoom_change, names="value")

        self.chk_workspace = widgets.Checkbox(
            value=False, description="Show Workspace Surface", indent=False
        )
        self.chk_workspace.observe(self._on_workspace_change, names="value")

        self.chk_track = widgets.Checkbox(
            value=True, description="Show Trajectory & Failures", indent=False
        )
        self.chk_track.observe(self._on_track_change, names="value")

        # --- Stroke readouts (single HTML block) ---
        self.lbl_strokes = widgets.HTML(
            value=self._build_strokes_html([0.0] * 6, [False] * 6)
        )

        # --- Position label ---
        self.lbl_pos = widgets.HTML(
            value="<b>X:</b> 0.0 | <b>Y:</b> 0.0 | <b>Z:</b> 0.0"
        )

        # --- Manual sliders ---
        self.sld_x = widgets.FloatSlider(
            value=0,
            min=-300,
            max=300,
            step=1,
        ...
        self.sld_y = widgets.FloatSlider(
            value=0,
            min=-300,
            max=300,
            step=1,
        ...
        self.sld_z = widgets.FloatSlider(
            value=0,
            min=-300,
            max=300,
            step=1,
        ...
        self.file_upload = widgets.FileUpload(
            accept=".csv,.txt", multiple=False, description="Load Data"
        )
        ...
        # --- Amplitude ---
        self.sld_amp = widgets.IntSlider(
            value=100,
            min=0,
            max=200,
            step=10,
        ...
        self.sld_progress = widgets.IntSlider(
            value=0,
            min=0,
            max=1,
            step=1,
        ...
        self.btn_precompute = widgets.Button(
            description="Precompute Workspaces", layout=widgets.Layout(width="100%")
        )
    ...
    def _build_strokes_html(strokes, over_limits):
        ...
        for i, (val, over) in enumerate(zip(strokes, over_limits)):
            ...
            lines.append(
                f"<span style='color:{color}'>Leg {i + 1}: {val:+.1f} mm</span>"
            )
        return (
            "<pre style='margin:0;font-size:13px;line-height:1.6;overflow:visible;max-height:none'>"
            + "\n".join(lines)
            + "</pre>"
        )
    ...
    def _build_layout(self):
        ...
        player_row = widgets.HBox(
            [self.player],
            layout=widgets.Layout(
                justify_content="center", overflow="visible", width="100%"
            ),
        )
        ...
        self.controls = widgets.VBox(
            ...
            layout=widgets.Layout(width="380px", padding="8px", overflow="visible"),
        )

        self.ui = widgets.HBox(
            [self.fig.canvas, self.controls],
    ...
    def _apply_view(self, mode):
        if "Default" in mode:
            self.ax_3d.view_init(elev=20, azim=-75)
        elif "Front" in mode:
    ...
    def update_platform(self, pos):
        ...
        plat_radius = np.mean(
            np.sqrt(self.P_local[:, 0] ** 2 + self.P_local[:, 1] ** 2)
        )
        ...
        for k in range(6):
            ...
            self.lines_legs[k].set_data(
                [self.B[k, 0], P_curr[k, 0]], [self.B[k, 1], P_curr[k, 1]]
            )
            ...
            u = leg_vec / L
            ang = np.degrees(
                np.arccos(np.clip(np.dot(u, self.Leg_Vec_Neutral[k]), -1, 1))
            )
        ...
        if warn_msg:
            self.lbl_status.value = (
                f'<span style="color:red">LIMIT WARNING: {warn_msg}</span>'
            )
        elif not self.is_playing:
            self.lbl_status.value = "Status: OK"

        self.lbl_pos.value = (
            f"<b>X:</b> {pos[0]:.1f} | <b>Y:</b> {pos[1]:.1f} | <b>Z:</b> {pos[2]:.1f}"
        )
        self.fig.canvas.draw_idle()
    ...
    def _on_file_upload(self, change):
        ...
        if isinstance(uploaded, dict):
            ...
            info = uploaded[0]
            content = getattr(
                info, "content", info["content"] if isinstance(info, dict) else None
            )
            name = getattr(
                info, "name", info["name"] if isinstance(info, dict) else "data"
            )
    ...
    def validate_trajectory(self):
        ...
        if len(fail_idx) > 0:
            ...
            self.scat_fail._offsets3d = ([], [], [])
        self.lbl_status.value = (
            f"Data: {len(self.loaded_data)} pts | Fail: {len(fail_idx)}"
        )
